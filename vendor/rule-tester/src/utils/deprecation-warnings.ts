...
/**
 * Emits a deprecation warning containing a given filepath. A new deprecation
 * warning is emitted for each unique file path, but repeated invocations with
 * the same file path have no effect. No warnings are emitted if the
 * `--no-deprecation` or `--no-warnings` Node runtime flags are active.
 *
