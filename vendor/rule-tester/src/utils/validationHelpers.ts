...
/**
 * Wraps the given parser in order to intercept and modify return values from
 * the `parse` and `parseForESLint` methods, for test purposes. In particular,
 * to modify ast nodes, tokens and comments to throw on access to their `start`
 * and `end` properties.
 */
export function wrapParser(parser: Linter.ParserModule): Linter.ParserModule {
	/**
	 * Define `start`/`end` properties of all nodes of the given AST as throwing
	 * error.
	 */
...
/**
 * Emit a deprecation warning if rule has options but is missing the
